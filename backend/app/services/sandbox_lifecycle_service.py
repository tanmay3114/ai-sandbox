...
class SandboxLifecycleService:
    ...
    def list_sandboxes(self, limit: int = 100, offset: int = 0) -> list[Sandbox]:
        """Retrieve all sandboxes, reconciling TTL expiration for active sessions."""
        sandboxes = self.sandbox_repo.list_sandboxes(limit=limit, offset=offset)
        for sandbox in sandboxes:
            if sandbox.is_expired and sandbox.is_active:
                logger.info(f"Sandbox {sandbox.id} expired during listing; executing cleanup.")
                self.expire_sandbox(sandbox)
                self.db.refresh(sandbox)
        return sandboxes

    def list_executions(self, sandbox_id: uuid.UUID) -> list[JobExecutionResponse]:
        """List all execution records for a specific sandbox session."""
        self.get_sandbox(sandbox_id)
        jobs = self.execution_repo.list_by_sandbox(sandbox_id)
        return [
            JobExecutionResponse(
                execution_id=job.id,
                sandbox_id=job.sandbox_id,
